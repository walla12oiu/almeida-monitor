...
def send_email(subject: str, body: str) -> None:
    ...
    html_body = f"""
    <html><body>
      <h2 style="color:#c0392b;">🎭 Almeida Ticket Alert!</h2>
      <p>{body}</p>
      <p><a href="{URL}" style="background:#c0392b;color:white;padding:10px 20px;
         text-decoration:none;border-radius:4px;">👉 Go to Almeida Calendar</a></p>
      <hr>
...
def main():
    ...
    try:
        ...
        if last_hash is None:
            # First ever run — just save the hash, don't alert
            save_hash(current_hash)
            print("  First run — baseline saved. No alert sent.")

        elif current_hash != last_hash:
            print("  ⚠️  CHANGE DETECTED — sending email...")
            send_email(
                subject="🎭 Almeida American Psycho — PAGE CHANGED (tickets may be live!)",
                body=(
        ...
        # send_email("Almeida Monitor — ERROR", str(e))
        raise
